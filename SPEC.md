...
- Topic-level dedup: a weekly-rolling "topics covered" list (e.g., "DSPy 2.5 release", "Anthropic's SWE-Bench result") is **planned but not yet implemented** — see #4. The `topics_covered` table exists in the DB schema but is never written to by the current pipeline. When implemented, it would be passed into the ranker prompt so it can down-weight items that are just the 4th take on the same news.
